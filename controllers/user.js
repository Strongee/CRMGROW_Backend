...
const daily_report = new CronJob({
  ...
  onTick: async function() {
    const users = User.find({daily_report: true})
    ...
    for( let i = 0; i< users.length; i ++){
      ...
      for (let j =0; j < activity.length; j ++){
        const contact = await Contact.findOne({_id: activity[j].contacts})
      ...
      for(let j = 0; j < _follow_up.length; j ++){
        const contact = await Contact.findOne({_id: _follow_up[j].contact}) 
...
const weekly_report = new CronJob({
  ...
  cronTime: '00 21 * * Sun',
  onTick: async function() {
    const users = User.find({weekly_report: true})
    sgMail.setApiKey(config.SENDGRID.SENDGRID_KEY);
