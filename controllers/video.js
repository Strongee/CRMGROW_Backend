...
const generatePreview = async (file_path) => {
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 1; i < 40; i++) {
      const image = await loadImage(`${GIF_PATH}screenshot-${i}.jpg`);
  ...
  if (contacts) {
    ...
    for(let i=0; i<contacts.length; i++){
      ...
      const msg = {
        ...
        subject: video_subject,
        html: `<html><head><title>Video Invitation</title></head><body><p style="white-space:pre-wrap;max-width: 800px;margin-top:0px;">${video_content}<br/>Thank you,<br/><br/>${currentUser.email_signature}</body></html>`,
        text: video_content,
      };
...
const bulkGmail = async (req, res) => {
  ...
  if (contacts) {
    ...
    for(let i=0; i<contacts.length; i++){
      ...
      const email_content = `<html><head><title>Video Invitation</title></head><body><p style="white-space:pre-wrap;max-width: 800px;margin-top:0px;">${video_content}<br/>Thank you,<br/><br/>${currentUser.email_signature}</body></html>`;
...
const bulkText = async (req, res) => {
  const { currentUser } = req;
  const { content, videos, contacts } = req.body;
  const promise_array = [];
  const error = [];
  if (contacts) {
    if (contacts.length > config.MAX_EMAIL) {
    ...
    for (let i = 0; i < contacts.length; i++) {
      ...
      const sendMail = {
        message: {
          ...
          body: {
            contentType: 'HTML',
            content: `<html><head><title>Video Invitation</title></head><body><p style="white-space:pre-wrap;max-width: 800px;margin-top:0px;">${video_content}<br/>Thank you,<br/><br/>${currentUser.email_signature}</body></html>`,
          },
          toRecipients: [
            {
              emailAddress: {
