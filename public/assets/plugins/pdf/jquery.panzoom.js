...
(function (global, factory) {
  ...
  if (window.PointerEvent) {
    ...
    $.each(list, function (i, name) {
      ...
      if (i < 2) {
        ...
        var touch =
          'touch' + (name === 'down' ? 'start' : name === 'up' ? 'end' : name);
        // Add fixHook
  ...
  function Panzoom(elem, options) {
    ...
    // De-camelcase
    this._transform =
      !this.isSVG && $.cssProps.transform.replace(rupper, '-$1').toLowerCase();
  ...
  Panzoom.prototype = {
    ...
    resetDimensions: function () {
      ...
      dims.widthBorder =
        $.css(elem, 'borderLeftWidth', true) +
          $.css(elem, 'borderRightWidth', true) || 0;
      dims.heightBorder =
        $.css(elem, 'borderTopWidth', true) +
          $.css(elem, 'borderBottomWidth', true) || 0;
    ...
    getTransform: function (transform) {
      ...
      if (transform) {
        ...
        transform = $[this.isSVG ? 'attr' : 'style'](
          transformElem,
          'transform'
        );
      }
    ...
    setMatrix: function (matrix, options) {
      ...
      var $parent = this.$parent;
      var contain =
        typeof options.contain !== 'undefined'
          ? options.contain
      ...
      if (contain) {
        ...
        if (contain === 'invert') {
          ...
          matrix[4] = Math.max(
            Math.min(matrix[4], marginW - left),
            -marginW - left - diffW
          );
          matrix[5] = Math.max(
            Math.min(matrix[5], marginH - top),
            -marginH - top - diffH + dims.heightBorder
          );
        } else {
          ...
          if (
            $parent.css('textAlign') !== 'center' ||
            !rinline.test($.css(this.elem, 'display'))
          ) {
            marginW = marginH = 0;
          } else {
            diffW = 0;
    ...
    transition: function (off) {
      ...
      var transition =
        off || !this.options.transition ? 'none' : this._transition;
      var $set = this.$set;
      var i = $set.length;
      while (i--) {
    ...
    zoom: function (scale, opts) {
      ...
      this.setMatrix(matrix, {
        animate:
          typeof options.animate === 'boolean' ? options.animate : animate,
    ...
    _bind: function () {
      ...
      var str_start = pointerEvents
        ? 'pointerdown' + ns
        : 'touchstart' + ns + ' mousedown' + ns;
      var str_click = pointerEvents
        ? 'pointerup' + ns
        : 'touchend' + ns + ' click' + ns;
      var events = {};
      ...
      if ($zoomRange.length) {
        $zoomRange
          .attr({
            ...
            step:
              (options.rangeStep === Panzoom.defaults.rangeStep &&
                $zoomRange.attr('step')) ||
              options.rangeStep,
      ...
      if ($zoomRange.length) {
        ...
        events[
          (pointerEvents ? 'pointerdown' : 'mousedown') + ns
        ] = function () {
    ...
    _buildTransition: function () {
      if (this._transform) {
        var options = this.options;
        this._transition =
          this._transform + ' ' + options.duration + 'ms ' + options.easing;
      }
    },
